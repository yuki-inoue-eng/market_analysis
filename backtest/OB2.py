...
class OB2Strategy(Strategy):
    ...
    def should_continue_buy(self):
        ...
        if self.sum_sell_power() == 0 or self.sum_sell_fuel() == 0:
            return False
        return self.sum_buy_power() / self.sum_sell_power() > params["exit_buy_power_threshold"] \
               and self.sum_buy_fuel() / self.sum_sell_fuel() > params["buy_fuel_threshold"]
    ...
    def should_continue_sell(self):
        ...
        if self.sum_buy_power() == 0 or self.sum_buy_fuel() == 0:
            return False
        return self.sum_sell_power() / self.sum_buy_power() > params["exit_sell_power_threshold"] \
               and self.sum_sell_fuel() / self.sum_buy_fuel() > params["sell_fuel_threshold"]
    ...
    def should_buy_entry(self):
        ...
        if self.sum_sell_power() == 0 or self.sum_sell_fuel() == 0:
            return False
        return self.sum_buy_power() / self.sum_sell_power() > params["enter_buy_power_threshold"] \
               and self.sum_buy_fuel() / self.sum_sell_fuel() > params["buy_fuel_threshold"]
    ...
    def should_sell_entry(self):
        ...
        if self.sum_buy_power() == 0 or self.sum_buy_fuel() == 0:
            return False
        return self.sum_sell_power() / self.sum_buy_power() > params["enter_sell_power_threshold"] \
           and self.sum_sell_fuel() / self.sum_buy_fuel() > params["sell_fuel_threshold"]
...
if __name__ == "__main__":
    ...
    order_books_json = open("../data/order_book/EUR_USD_OB_2018-01-01_2019-01-01.json")
    order_books = json.load(order_books_json)
    position_books_json = open("../data/position_book/EUR_USD_PB_2018-01-01_2019-01-01.json")
    position_books = json.load(position_books_json)
    # read candles from csv file.
    with open("../data/candles/EUR_USD_1T_2018-01-01_2019-01-01.csv", "r") as read_obj:
        feed = Cerebro.convert_feed_to_candles(list(reader(read_obj)), date_from, date_to)

    params = {
        "enter_buy_power_threshold": 2.0,  # entry 条件
        "exit_buy_power_threshold": 1.0,  # exit 条件
        "buy_fuel_threshold": 1.5,
        "enter_sell_power_threshold": 1.4,  # entry 条件
        "exit_sell_power_threshold": 1.0,  # exit 条件
        "sell_fuel_threshold": 2.7,
    ...
    # back_test
    my_strategy = OB2Strategy(instrument, params, order_books, position_books)
    start = time.time()
    print("Start back test.")
    cerebro = Cerebro(feed, my_strategy, 0, True)
    cerebro.result_dir_name = "OB1_EUR_USD_{}-{}".format(d_from, d_to)
    cerebro.run()
