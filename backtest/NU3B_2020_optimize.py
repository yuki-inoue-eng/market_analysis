...
class NU3BStrategy(Strategy):
    def __init__(self, order_books: dict, position_book: dict, params: dict):
        super().__init__()
...
if __name__ == "__main__":
    ...
    param_ranges = {
        "spreadAllowance": {"min": -2.5, "max": -1.5, "step": 0.1},
        "profitDistance": {"min": 11, "max": 16, "step": 1},
        "stopLossDistance": {"min": 6, "max": 13, "step": 1},
        "stopOrders_1": {"min": 0.7, "max": 0.8, "step": 0.1},
        "stopOrders_2": {"min": 1.1, "max": 1.2, "step": 0.1},
        "stopOrders_3": {"min": 1.1, "max": 1.2, "step": 0.1},
    }

    optimizer = Optimizer(order_books, None, feed, param_ranges, NU3BStrategy)
